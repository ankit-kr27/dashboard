...
app.get("/api/v1/stats_data", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM assignment_data_1");
...
app.get("/api/v1/feedback_data", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM assignment_data_5");
    res.json(result.rows[0]);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Database error" });
...
app.get("/api/v1/score_data", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM assignment_data_3");
    res.json(result.rows[0]);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Database error" });
